...
public class DriveCommand extends Command {
    ...
    private SlewRateLimiter speedRateLimiter = new SlewRateLimiter(2.0);
    ...
    public DriveCommand(DriveSubsystem swerveSubsystem, XboxController xboxController) {
    ...
    public void execute() {
        ...
        ChassisSpeeds chassisSpeeds = ChassisSpeeds.fromFieldRelativeSpeeds(
                ...
                new Rotation2d(-swerveSubsystem.getRotation2d()
                        .rotateBy(DriveConstants.NAVX_ANGLE_OFFSET).getRadians()));

        switch (state) {
            case Free:
                state = xboxController.getBButton() ? DriveState.Locked : DriveState.Free;
                break;
            case Locked:
                state = ((joystickInput.getNorm() > 0.15) && !xboxController.getBButton()) ? DriveState.Free : DriveState.Locked;
                break;
        }

        SwerveModuleState[] calculatedModuleStates = DriveConstants.KINEMATICS.toSwerveModuleStates(chassisSpeeds);
        swerveSubsystem.setModuleStates(calculatedModuleStates);
    }
